...
export class ChipsListComponent implements OnInit, ControlValueAccessor {
  ...
  ngOnInit() {
    ...
    this.memberResults$ = this.form.get('memberSearch').valueChanges.pipe(debounceTime(300)).pipe(distinctUntilChanged()).pipe(filter(s => s && s.length > 1)).pipe(switchMap(str => this.service.searchUsers(str)));
  }
